...
procedure ListarAdicionalPorId(Req: THorseRequest; Res: THorseResponse; Next: TProc);
implementation
...
begin
  ...
  THorse{.AddCallback(HorseJWT(Controllers.Auth.SECRET, THorseJWTConfig.New.SessionClass(TMyClaims)))}
        .get('/adicionais/:id_adcional', ListarAdicionalPorId);
...
begin
  ...
  try
    try
      ...
      LId        := Req.Params['id_adcional'].ToInteger;

      Res.Send<TJSONObject>(LDAOAdicionais.ListarAdicionalPorId(LId));
      Res.Status(LDAOAdicionais.CodigoResposta);
