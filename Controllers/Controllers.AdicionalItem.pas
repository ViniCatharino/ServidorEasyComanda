...
procedure ListarAdicionalItem(Req: THorseRequest; Res: THorseResponse; Next: TProc);
implementation
...
begin
  ...
  THorse{.AddCallback(HorseJWT(Controllers.Auth.SECRET, THorseJWTConfig.New.SessionClass(TMyClaims)))}
        .get('/adicionais/itens-adicionais', ListarAdicionalItem);
...
var
  ...
  LIdItem           : Integer;
begin
  LDAOAdicionalItem := nil;
  try
    try
      LDAOAdicionalItem := TDAOAdicionalItem.Create;

      LJSONBody := Req.Body<TJSONObject>;
      LIdItem   := Req.Params['id_pedido'].ToInteger;

      Res.Send<TJSONArray>(LDAOAdicionalItem.ListarAdicionalPorIdItem(lIdItem));
...
begin
  ...
  try
    try
      ...
      Res.Send<TJSONArray>(LDAOAdicionalItem.ListarAdicional);
      Res.Status(LDAOAdicionalItem.CodigoResposta);
...
begin
  ...
  try
    try
      ...
      LDAOAdicionalItem.EditarAdicional(LIdPedido, LIdAdicional, LQuantidade, LValor) ;
      Res.Status(LDAOAdicionalItem.CodigoResposta);
    except on ex:exception do
      Res.Send(ex.Message).Status(500);
    end;
